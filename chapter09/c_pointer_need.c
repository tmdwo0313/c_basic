...
/**
 * 포인터를 사용하는 이유
 *  - 변수를 사용하는 가장 쉬운방법은 변수이름 호출
 *    (int num = 4;  →  num) (편함)
 *  - 포인터 사용 → 추가적으로 변수선언, 주소연산,
 *                 간접참조 등 각종연산 수행해야함
 *    (불편)
 *  - 포인터를 즐겨 사용할 필요 없음!
 *  - 임베디드 프로그래밍 할 때 메모리에 직접 접근하는
 *    경우 똔느 동적 할당 메모리를 사용하는 경우에는
 *    포인터가 반드시 필요함
*/
 ...
 * 실제로 바뀌는 값은 main()함수의 변수 a와 b
 * swap()함수는 포인터를 통해 main()함수의 변수 a, b를 공유
 * (두 변수를 직접 바꾸는 일 가능)
 * → C언어가 Call by value기 때문에 포인터를 사용하지 않고
 *   값을 전달하며 값이 copy돼서 전달되기 때문에 공유 불가!
*/
